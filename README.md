...
Для мониторинга конкурентов дополнительно скачать браузер (разово):
...
src/competitors/           # мониторинг меню и цен конкурентов
├── storage.py             # локальный SQLite (data/competitors.db)
├── fetcher.py             # Playwright: снятие текста меню
├── extractor.py           # LLM: текст меню → JSON позиций
├── comparator.py          # диффы цен (чистый Python)
└── service.py             # оркестратор еженедельной проверки

scripts/
├── test_calc.py           # тест калькулятора
├── test_llm.py            # REPL для LLM
├── recon_competitors.py   # разведка сайтов конкурентов
├── seed_competitors.py    # стартовый список конкурентов
└── check_competitors.py   # ручной прогон мониторинга без Telegram
```

## Мониторинг конкурентов

Раз в неделю (день/время — `COMPETITORS_CHECK_*` в `.env`) бот снимает меню
с сайтов конкурентов, сравнивает с прошлым срезом и присылает сводку в Telegram,
если цены существенно изменились (порог: `max(10%, 30 ₽)`, настраивается).

Команды: `/list_competitors`, `/add_competitor <url>`, `/remove_competitor <url>`,
`/check_competitors` (проверить сейчас), `/competitors_report` (ссылка на таблицу).

Отчёты выгружаются в **отдельную** Google-таблицу: создай пустую таблицу, расшарь
на email сервисного аккаунта (роль «Редактор») и пропиши её ID в
`COMPETITORS_SHEETS_ID`. Без этой переменной всё работает, просто без выгрузки.

Если сайт не пускает бота (например, Бургер Кинг) — сохрани страницу меню
в браузере (Ctrl+S, «Веб-страница, только HTML» или .mhtml) и пришли файл боту
с подписью-названием конкурента: он разберёт меню из файла.
